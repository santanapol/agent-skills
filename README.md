...
3. ใน Agent chat พิมพ์ `/` แล้วเลือกคำสั่ง เช่น `/spec`, `/plan`, `/code-build`, `/review`, `/ship`
...
| `/code-build` | ลงมือ implement ทีละส่วน | incremental-implementation + test-driven-development |
...
Slash wrappers (`spec/`, `plan/`, `code-build/` ฯลฯ) ชี้ไป lifecycle skill หลัก — ดู mapping ใน [.cursor/skills/README.md](.cursor/skills/README.md)
